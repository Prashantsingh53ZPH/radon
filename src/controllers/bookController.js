...
const bookList= async function (req, res) {
    let data2= await BookModel.find({}).select({bookName: 1, authorName:1, _id:0})
    res.send({msg: data2})
}
...
const getBooksInYear = async function (req, res) {
    ...
    let data3 = await BookModel.find({ year : Year })
    res.send({msg: data3})
}

const getParticularBooks= async function(req, res){
    let input= req.body
    let data4=await BookModel.find(input)
    res.send({msg : data4})

}

const getXINRBooks= async function(req,res){
    let data5= await BookModel.find({ $or: [{"prices.indianPrice" :{$eq:500 }}, {"prices.indianPrice": {$eq: 1000}}] })
    res.send({msg: data5})
}

const getRandomBooks =async function(req,res){
    let data6= await BookModel.find({ $or : [{stockAvailable :false, totalPages : { $gte : 500} }]  })
    res.send({msg: data6})
}
